        ...
        <div class="time">
            <div class="hour">
                <a href="#" v-for="(item, index) in hourList" v-if="item < 10"
                   @click="initMap(month,day,item)" :class="{'hour-selected': hour===item}"> 0{{item}}:00 </a>
                <a href="#" v-else="item >= 10" @click="initMap(month,day,item)"
                   :class="{'hour-selected': hour===item}">{{item}}:00</a>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            showChild(data) {
                ...
                this.day = data.day
                this.initMap(this.month, this.day, new Date().getHours())
            },
            initMap(month, day, hour) {
                map = null
                let params = {
                    mapCenter: [104.065735, 30.659462],
                ...
                axios.get('http://localhost:3000/weathers/kri-hour', { params: param }).then((weaRes) => {
                    let weaArr = weaRes.data.result
                    console.log(weaArr)
                    weaArr.forEach((item) => {
                        let feature = new Feature({
                            // geometry: new ol.geom.Point([data.features[i].attributes.x, data.features[i].attributes.y]),
                            geometry: new Point([item.docs[0].lon, item.docs[0].lat]),
                            // value: data.features[i].properties.centroid[0]
                            value: item.TEM
                        })
                    ...
                    const clipgeom = new Polygon(coord)
                    // 绘制kriging插值图
                    let canvasLayer = null
                    const drawKriging = function(extent) {
                        const values = [], lngs = [], lats = []
                        const l = WFSVectorSource.l
